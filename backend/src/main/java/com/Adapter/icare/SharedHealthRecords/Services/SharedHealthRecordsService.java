...
public class SharedHealthRecordsService {
            ...
            if (!response.getEntry().isEmpty()) {
                for (Bundle.BundleEntryComponent entry : response.getEntry()) {
                    if (entry.getResource() instanceof Patient) {
                        ...
                        if (hfrCode != null) {
                            try {
                                ...
                                bundle = fhirClient.search().forResource(Organization.class)
                                        .where(Organization.IDENTIFIER.exactly().identifier(hfrCode))
                                        .returnBundle(Bundle.class).execute();
                        ...
                        // TODO: Add logic to handle number of visits. Latest visit is primary and the
                        // rest is history
                        if (referralNumber == null) {
                            encounters = getLatestEncounterUsingPatientAndOrganisation(
                                    patient.getIdElement().getIdPart(), organization, numberOfVisits);
                        }
                        if (!encounters.isEmpty()) {
                            for (Encounter encounter : encounters) {
                                ...
                                organization = fhirClient.read().resource(Organization.class)
                                        .withId(organisationReference.getIdPart()).execute();
                                // Bundle ouBundle =
                                ...
                                List<Coverage> coverages = this.clientRegistryService
                                        .getCoverages(patient.getIdElement().getIdPart());
                                ...
                                if (!coverages.isEmpty()) {
                                    visitMainPaymentDetailsDTOS = coverages.stream()
                                            .map(coverage -> this.clientRegistryService
                                                    .mapToMainVisitPaymentDetails(coverage))
                                            .collect(Collectors.toList());
                                }
                                // patientDTO.setPaymentDetails(paymentDetailsDTOs);
                                templateData.setVisitMainPaymentDetails(
                                        !visitMainPaymentDetailsDTOS.isEmpty() ? visitMainPaymentDetailsDTOS.get(0)
                                                : null);
                                String nationality = getNestedExtensionValueString(patient,
                                        "http://fhir.moh.go.tz/fhir/StructureDefinition/patient-extensions",
                                        "nationality");
                                String occupation = getNestedExtensionValueString(patient,
                                        "http://fhir.moh.go.tz/fhir/StructureDefinition/patient-extensions",
                                        "occupation");
                                ...
                                visitDetails.setClosedDate(
                                        encounter.getPeriod() != null && encounter.getPeriod().getEnd() != null
                                ...
                                for (Observation careServiceObs : careServicesObs) {
                                    ...
                                    if (careServiceObs.hasComponent() && !careServiceObs.getComponent().isEmpty()) {
                                        Observation.ObservationComponentComponent careTypeComponent = careServiceObs
                                                .getComponent().get(0);
                                        if (careTypeComponent != null && careTypeComponent.hasValueStringType()
                                                && careTypeComponent.getValueStringType().hasValue()) {
                                            careServiceDTO.setCareType(
                                                    careTypeComponent.getValueStringType().getValueAsString());
                                        }
                                        if (careServiceObs.getComponent().size() > 1) {
                                            ...
                                            if (visitNumberComponent != null
                                                    && visitNumberComponent.hasValueIntegerType()
                                                    && visitNumberComponent.getValueIntegerType().hasValue()) {
                                                careServiceDTO.setVisitNumber(
                                                        visitNumberComponent.getValueIntegerType().getValue());
                                            }
                                        }
                                    }
                                ...
                                List<Observation> observationGroups = getObservationsByCategory("vital-signs",
                                ...
                                for (Observation observationGroup : observationGroups) {
                                    List<Observation> observationsData = getObservationsByObservationGroupId(
                                            "vital-signs", encounter, observationGroup.getIdElement().getIdPart());
                                    if (!observationsData.isEmpty()) {
                                        Map<String, Object> vitalSign = new LinkedHashMap<>();
                                        for (Observation observation : observationsData) {
                                            // TODO: Improve the code to use dynamically fetched LOINC codes for vital
                                            // signs
                                            if (observation.hasCode() && observation.getCode().getCoding().get(0)
                                                    .getCode().equals("85354-9")) {
                                            ...
                                            if (observationGroup.hasEffectiveDateTimeType()
                                                    && observationGroup.getEffectiveDateTimeType().getValue() != null) {
                                                SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                                                String formattedDate = sdf
                                                        .format(observationGroup.getEffectiveDateTimeType().getValue());
                                ...
                                List<Observation> visitNotesGroup = getObservationsByCategory("visit-notes", encounter,
                                        true, false);
                                // Visit notes
                                    ...
                                    for (Observation observationGroup : visitNotesGroup) {
                                        ...
                                        if (!chiefComplaintsData.isEmpty()) {
                                            for (Observation observation : chiefComplaintsData) {
                                                chiefComplaints.add(observation.hasValueStringType()
                                        ...
                                        List<Observation> historyOfPresentIllnessData = getObservationsByObservationGroupId(
                                                "history-of-preventive-illness", encounter,
                                                observationGroup.getIdElement().getIdPart());
                                        if (!historyOfPresentIllnessData.isEmpty()) {
                                            for (Observation observation : historyOfPresentIllnessData) {
                                                historyOfPresentIllness.add(observation.hasValueStringType()
                                                        ? observation.getValueStringType().toString()
                                                        : null);
                                            }
                                        }
                                        ...
                                        List<Observation> reviewOfOtherSystemsData = getObservationsByObservationGroupId(
                                                "review-of-other-system", encounter,
                                                observationGroup.getIdElement().getIdPart());
                                        if (!reviewOfOtherSystemsData.isEmpty()) {
                                            for (Observation observation : reviewOfOtherSystemsData) {
                                                ...
                                                data.put("code",
                                                        observation.hasCode()
                                                                ? observation.getCode().getCoding().get(0).getCode()
                                                                        .toString()
                                                                : null);
                                                data.put("name",
                                                        observation.hasCode()
                                                                ? observation.getCode().getCoding().get(0).getDisplay()
                                                                : null);
                                        ...
                                        List<Observation> pastMedicalHistoryData = getObservationsByObservationGroupId(
                                                "past-medical-history", encounter,
                                                observationGroup.getIdElement().getIdPart());
                                        if (!pastMedicalHistoryData.isEmpty()) {
                                            for (Observation observation : pastMedicalHistoryData) {
                                                pastMedicalHistory.add(observation.hasValueStringType()
                                                        ? observation.getValueStringType().toString()
                                                        : null);
                                            }
                                        }
                                        ...
                                        List<Observation> familyAndSocialHistoryData = getObservationsByObservationGroupId(
                                                "family-and-social-history", encounter,
                                                observationGroup.getIdElement().getIdPart());
                                        if (!familyAndSocialHistoryData.isEmpty()) {
                                            for (Observation observation : familyAndSocialHistoryData) {
                                                familyAndSocialHistory.add(observation.hasValueStringType()
                                                        ? observation.getValueStringType().toString()
                                                        : null);
                                            }
                                        }
                                        ...
                                        List<Observation> generalExaminationObservationData = getObservationsByObservationGroupId(
                                                "general-examination", encounter,
                                                observationGroup.getIdElement().getIdPart());
                                        if (!generalExaminationObservationData.isEmpty()) {
                                            for (Observation observation : generalExaminationObservationData) {
                                                generalExaminationObservation.add(observation.hasValueStringType()
                                                        ? observation.getValueStringType().toString()
                                                        : null);
                                            }
                                        }
                                        visitNotesData.put("generalExaminationObservation",
                                        ...
                                        List<Observation> systemicExaminationObservationData = getObservationsByObservationGroupId(
                                                "systemic-examination", encounter,
                                                observationGroup.getIdElement().getIdPart());
                                        ...
                                        visitNotesData.put("systemicExaminationObservation",
                                        ...
                                        List<Observation> doctorPlanOrSuggestionData = getObservationsByObservationGroupId(
                                                "doctor-plan", encounter, observationGroup.getIdElement().getIdPart());
                                        if (!doctorPlanOrSuggestionData.isEmpty()) {
                                            for (Observation observation : doctorPlanOrSuggestionData) {
                                                doctorPlanOrSuggestion.add(observation.hasValueStringType()
                                                        ? observation.getValueStringType().toString()
                                                        : null);
                                            }
                                        }
                                ...
                                if (!allergyIntolerances.isEmpty()) {
                                    for (AllergyIntolerance allergyIntolerance : allergyIntolerances) {
                                        if (allergyIntolerance.hasCode()) {
                                            ...
                                            allergiesDTO.setCode(allergyIntolerance.hasCode()
                                                    && allergyIntolerance.getCode().hasCoding()
                                                    && !allergyIntolerance.getCode().getCoding().isEmpty()
                                                            ? allergyIntolerance.getCode().getCoding().get(0).getCode()
                                                                    .toString()
                                                            : null);
                                            allergiesDTO.setCategory(allergyIntolerance.hasCategory()
                                                    && !allergyIntolerance.getCategory().isEmpty()
                                                            ? allergyIntolerance.getCategory().get(0).getCode()
                                                            : null);
                                            allergiesDTO.setName(allergyIntolerance.hasCode()
                                                    && allergyIntolerance.getCode().hasCoding()
                                                    && !allergyIntolerance.getCode().getCoding().isEmpty()
                                                            ? allergyIntolerance.getCode().getCoding().get(0)
                                                                    .getDisplay()
                                                            : null);
                                            allergiesDTO.setCriticality(allergyIntolerance.hasCriticality()
                                                    ? allergyIntolerance.getCriticality().getDisplay().toString()
                                                    : null);
                                            allergiesDTO
                                                    .setVerificationStatus(allergyIntolerance.hasVerificationStatus()
                                                            && allergyIntolerance.getVerificationStatus().hasCoding()
                                                                    ? allergyIntolerance.getVerificationStatus()
                                                                            .getCoding().get(0).getCode().toString()
                                                                    : null);
                                ...
                                List<Condition> conditions = getConditionsByCategory(
                                        encounter.getIdElement().getIdPart(), "chronic-condition");
                                if (!conditions.isEmpty()) {
                                    for (Condition condition : conditions) {
                                        ChronicConditionsDTO chronicConditionsDTO = new ChronicConditionsDTO();
                                        chronicConditionsDTO
                                                .setCode(condition.hasCode() && condition.getCode().hasCoding()
                                                        && !condition.getCode().getCoding().isEmpty()
                                                        && condition.getCode().getCoding().get(0).hasCode()
                                                                ? condition.getCode().getCoding().get(0).getCode()
                                                                : null);
                                        chronicConditionsDTO.setName(condition.hasCategory()
                                                && !condition.getCategory().isEmpty()
                                                && condition.getCategory().get(0).getCoding().get(0).hasCode()
                                                        ? condition.getCategory().get(0).getCoding().get(0).getCode()
                                                        : null);
                                        chronicConditionsDTO
                                                .setName(condition.hasCode() && condition.getCode().hasCoding()
                                                        && !condition.getCode().getCoding().isEmpty()
                                                        && condition.getCode().getCoding().get(0).hasDisplay()
                                                                ? condition.getCode().getCoding().get(0).getDisplay()
                                                                : null);
                                        chronicConditionsDTO.setCriticality(condition.hasClinicalStatus()
                                                && condition.getClinicalStatus().hasCoding()
                                                && !condition.getClinicalStatus().getCoding().isEmpty()
                                                && condition.getClinicalStatus().getCoding().get(0).hasCode()
                                                        ? condition.getClinicalStatus().getCoding().get(0).getCode()
                                                        : null);
                                        chronicConditionsDTO.setVerificationStatus(condition.hasVerificationStatus()
                                                && condition.getVerificationStatus().getCoding().get(0).hasCode()
                                                        ? condition.getVerificationStatus().getCoding().get(0).getCode()
                                                        : null);
                                    ...
                                    for (Observation observation : smokingObs) {
                                        if (observation.hasValue() && observation.hasValueBooleanType()) {
                                            ...
                                            smoking.put("notes", observation.getNote().stream()
                                                    .map(note -> note.getText()).collect(Collectors.joining(", ")));
                                            break;
                                        }
                                    }
                                }
                                ...
                                List<Observation> alcoholUseObs = getObservationsByCategory("alcohol-use", encounter,
                                        true, false);
                                ...
                                List<Condition> conditionsList = getConditionsByCategory(
                                        encounter.getIdElement().getIdPart(), "encounter-diagnosis");
                                if (!conditionsList.isEmpty()) {
                                    for (Condition condition : conditionsList) {
                                        DiagnosisDetailsDTO diagnosisDetailsDTO = new DiagnosisDetailsDTO();
                                        diagnosisDetailsDTO.setDiagnosisCode(condition.hasCode()
                                                ? condition.getCode().getCoding().get(0).getCode().toString()
                                                : null);
                                        diagnosisDetailsDTO.setDiagnosis(condition.hasCode()
                                                ? condition.getCode().getCoding().get(0).getDisplay()
                                                : null);
                                        diagnosisDetailsDTO.setDiagnosisDate(condition.hasOnsetDateTimeType()
                                                ? condition.getOnsetDateTimeType().getValue()
                                                : null);
                                        diagnosisDetailsDTO.setDiagnosisDescription(
                                                condition.hasCode() ? condition.getCode().getText() : null);
                                        diagnosisDetailsDTO.setCertainty(condition.hasVerificationStatus()
                                                ? condition.getVerificationStatus().getCoding().get(0).getCode()
                                                : null);
                                ...
                                List<Observation> investigationDetailsGroup = getObservationsByCategory(
                                        "investigation-details", encounter, true, false);
                                // Visit notes
                                if (!investigationDetailsGroup.isEmpty()) {
                                    for (Observation observationGroup : investigationDetailsGroup) {
                                        ...
                                        investigationDetailsDTO
                                                .setDateOccurred(observationGroup.hasEffectiveDateTimeType()
                                                        ? observationGroup.getEffectiveDateTimeType().getValue()
                                                        : null);

                                        List<Observation> daysSinceSymptomsData = getObservationsByObservationGroupId(
                                                "days-since-symptoms", encounter,
                                                observationGroup.getIdElement().getIdPart());
                                        if (!daysSinceSymptomsData.isEmpty()) {
                                            for (Observation observation : daysSinceSymptomsData) {
                                                if (observation.hasValueQuantity()) {
                                                    investigationDetailsDTO.setDaysSinceSymptoms(
                                                            observation.getValueQuantity().getValue().intValueExact());
                                                    break;
                                                }
                                            }
                                        }
                                        ...
                                        List<Observation> diseaseCodeData = getObservationsByObservationGroupId(
                                                "disease-code", encounter, observationGroup.getIdElement().getIdPart());
                                        if (!diseaseCodeData.isEmpty()) {
                                            for (Observation observation : diseaseCodeData) {
                                                if (observation.hasValueCodeableConcept()) {
                                                    investigationDetailsDTO.setDiseaseCode(observation
                                        ...
                                        List<Observation> vaccinatedData = getObservationsByObservationGroupId(
                                                "vaccinated", encounter, observationGroup.getIdElement().getIdPart());
                                        if (!vaccinatedData.isEmpty()) {
                                            for (Observation observation : vaccinatedData) {
                                                if (observation.hasValueStringType()) {
                                                    investigationDetailsDTO
                                                            .setVaccinated(observation.getValueStringType().toString());
                                                    break;
                                                }
                                            }
                                        }
                                ...
                                List<DiagnosticReport> diagnosticReports = getDiagnosticReportsByCategory(
                                        encounter.getIdElement().getIdPart(), "laboratory");
                                    ...
                                    for (DiagnosticReport diagnosticReport : diagnosticReports) {
                                        ...
                                        labInvestigationDetailsDTO.setTestCode(diagnosticReport.hasCode() &&
                                                ...
                                                !diagnosticReport.getCode().getCoding().isEmpty()
                                                        ? diagnosticReport.getCode().getCoding().get(0).getCode()
                                                        : null);

                                        labInvestigationDetailsDTO
                                                .setTestResultDate(diagnosticReport.hasEffectiveDateTimeType()
                                                        && diagnosticReport.getEffectiveDateTimeType().hasValue()
                                                                ? diagnosticReport.getEffectiveDateTimeType().getValue()
                                                                : null);
                                        labInvestigationDetailsDTO.setTestStatus(getNestedExtensionValueString(
                                                diagnosticReport,
                                        ...
                                        for (Identifier reportIdentifier : identifiers) {
                                            if (reportIdentifier.hasValue() && reportIdentifier.hasType()
                                                    && reportIdentifier.getType().hasCoding()
                                                    && !reportIdentifier.getType().getCoding().isEmpty()) {
                                                if (reportIdentifier.getType().getCoding().get(0).getCode()
                                                        .equals("TEST-ORDER")) {
                                                    labInvestigationDetailsDTO
                                                            .setTestOrderId(reportIdentifier.getValue());
                                                } else if (reportIdentifier.getType().getCoding().get(0).getCode()
                                                        .equals("SAMPLE-ID")) {
                                                    labInvestigationDetailsDTO
                                                            .setTestSampleId(reportIdentifier.getValue());
                                                }
                                            }
                                        }
                                        labInvestigationDetailsDTO
                                                .setTestOrderDate(diagnosticReport.hasEffectiveDateTimeType()
                                                        ? diagnosticReport.getEffectiveDateTimeType().getValue()
                                                        : null);
                                        ...
                                        labInvestigationDetailsDTO.setTestType(diagnosticReport.hasCode() &&
                                                diagnosticReport.getCode().hasCoding() &&
                                                !diagnosticReport.getCode().getCoding().isEmpty()
                                                        ? diagnosticReport.getCode().getCoding().get(0).getDisplay()
                                                        : null);

                                        labInvestigationDetailsDTO.setStandardCode(
                                                getNestedExtensionValueBoolean(diagnosticReport,
                                                        "http://fhir.moh.go.tz/fhir/StructureDefinition/diagonostic-report-results",
                                                        "standardCode"));
                                        labInvestigationDetailsDTO.setCodeType(
                                                diagnosticReport.hasCode() &&
                                                        diagnosticReport.getCode().hasCoding() &&
                                                        !diagnosticReport.getCode().getCoding().isEmpty()
                                                        && diagnosticReport.getCode().getCoding().get(0).getSystem()
                                                                .contains("loinc") ? "LOINC" : null);
                                            ...
                                            for (Reference reference : diagnosticReport.getResult()) {
                                                ...
                                                if (obsReference.getResourceType().equals("Observation")) {
                                                    Observation observation = fhirClient.read()
                                                            ...
                                                            .withId(obsReference.getIdPart()).execute();
                                                    labTestResultsDTO.setStandardCode(
                                                            getNestedExtensionValueBoolean(observation,
                                                                    "http://fhir.moh.go.tz/fhir/StructureDefinition/laboratory-results",
                                                                    "standardCode"));
                                                    labTestResultsDTO
                                                            .setReleaseDate(observation.hasEffectiveDateTimeType()
                                                    ...
                                                    labTestResultsDTO.setResult(
                                                            observation.hasInterpretation()
                                                                    && !observation.getInterpretation().isEmpty()
                                                    ...
                                                    labTestResultsDTO.setCodedValue(
                                                            observation.hasCode() && observation.getCode().hasText()
                                                                    ? observation.getCode().getText()
                                                                    : null);
                                                    labTestResultsDTO.setHighRange(getNestedExtensionValueString(
                                                            observation,
                                                            "http://fhir.moh.go.tz/fhir/StructureDefinition/laboratory-results",
                                                            "highRange"));
                                                    labTestResultsDTO.setLowRange(getNestedExtensionValueString(
                                                            observation,
                                                            "http://fhir.moh.go.tz/fhir/StructureDefinition/laboratory-results",
                                                            "lowRange"));
                                                    labTestResultsDTO.setRemarks(getNestedExtensionValueString(
                                                            observation,
                                                            "http://fhir.moh.go.tz/fhir/StructureDefinition/laboratory-results",
                                                            "remarks"));
                                                    labTestResultsDTO.setCodeType(getNestedExtensionValueString(
                                                            observation,
                                                            "http://fhir.moh.go.tz/fhir/StructureDefinition/laboratory-results",
                                                            "codeType"));
                                                    labTestResultsDTO.setParameter(getNestedExtensionValueString(
                                                            observation,
                                                            "http://fhir.moh.go.tz/fhir/StructureDefinition/laboratory-results",
                                                            "parameter"));
                                                    labTestResultsDTO.setUnit(getNestedExtensionValueString(observation,
                                                            "http://fhir.moh.go.tz/fhir/StructureDefinition/laboratory-results",
                                                            "unit"));
                                                }
                                ...
                                List<ServiceRequest> serviceRequests = getServiceRequestsByCategory(
                                        encounter.getIdElement().getIdPart(), "referral-request");
                                if (!serviceRequests.isEmpty()) {
                                    for (ServiceRequest serviceRequest : serviceRequests) {
                                        if (serviceRequest.hasIdentifier()
                                                ...
                                                && !serviceRequest.getIdentifier().get(0).getType().getCoding()
                                                        .isEmpty()
                                                && serviceRequest.getIdentifier().get(0).getType().getCoding().get(0)
                                                        .getCode().equals("REFERRAL-NUMBER")) {
                                            referralDetailsDTO.setReferralDate(
                                                    serviceRequest.hasAuthoredOn() ? serviceRequest.getAuthoredOn()
                                                            : null);
                                            referralDetailsDTO.setReferralNumber(serviceRequest.hasIdentifier()
                                                    && !serviceRequest.getIdentifier().isEmpty()
                                                            ? serviceRequest.getIdentifier().get(0).getValue()
                                                                    .replaceFirst(Pattern.quote(orgCode + "-"), "")
                                                            : null);
                                            ...
                                            for (Reference reasonReference : serviceRequest.getReasonReference()) {
                                                ...
                                                if (observationReference.getResourceType().equals("Observation")) {
                                                    Observation observation = fhirClient.read()
                                                            .resource(Observation.class)
                                                            .withId(observationReference.getIdPart()).execute();
                                            ...
                                            for (Reference reference : performers) {
                                                if (reference.hasReferenceElement()
                                                        && reference.getType().equals("Organization")) {
                                                    IIdType performerReference = reference.getReferenceElement();
                                                    Organization performer = fhirClient.read()
                                                            .resource(Organization.class)
                                                            .withId(performerReference.getIdPart()).execute();
                                            ...
                                            Practitioner practitioner = fhirClient.read().resource(Practitioner.class)
                                                    .withId(practitionerReferenceType.getIdPart()).execute();
                                            referringClinician.put("MCTCode",
                                                    practitioner.hasIdentifier()
                                                            ? practitioner.getIdElement().getIdPart()
                                                            : null);
                                            referringClinician.put("name",
                                                    practitioner.hasName() && !practitioner.getName().isEmpty()
                                                            ? practitioner.getName().get(0).getText()
                                                            : null);
                                            referringClinician.put("phoneNumber",
                                                    practitioner.hasTelecom() && !practitioner.getTelecom().isEmpty()
                                ...
                                if (!medicationDispensesList.isEmpty()) {
                                    for (MedicationDispense medicationDispense : medicationDispensesList) {
                                        ...
                                        medicationDetailsDTO.setCode(medicationDispense.hasMedicationCodeableConcept()
                                                && medicationDispense.getMedicationCodeableConcept().hasCoding()
                                                        ? medicationDispense.getMedicationCodeableConcept().getCoding()
                                                                .get(0).getCode()
                                                        : null);
                                        medicationDetailsDTO.setName(medicationDispense.hasMedicationCodeableConcept()
                                                && medicationDispense.getMedicationCodeableConcept().hasCoding()
                                                        ? medicationDispense.getMedicationCodeableConcept().getCoding()
                                                                .get(0).getDisplay()
                                                        : null);
                                        medicationDetailsDTO.setOrderDate(medicationDispense.hasWhenHandedOver()
                                                ? medicationDispense.getWhenHandedOver()
                                                : null);
                                        String code = medicationDispense.hasMedicationCodeableConcept()
                                                && medicationDispense.getMedicationCodeableConcept().hasCoding()
                                                        ? medicationDispense.getMedicationCodeableConcept().getCoding()
                                                                .get(0).getSystem()
                                                        : null;
                                        String codeStandard = code.contains("msd") ? "MSD CODE"
                                                : code.contains("loinc") ? "LOINC" : null;
                                        medicationDetailsDTO.setCodeStandard(codeStandard);
                                        String duration = "";
                                        if (medicationDispense.getDaysSupply() != null) {
                                            duration = medicationDispense.getDaysSupply().getValue() + " "
                                                    + medicationDispense.getDaysSupply().getUnit();
                                        }
                                        medicationDetailsDTO.setPeriodOfMedication(duration);
                                        medicationDetailsDTO.setTreatmentType(
                                                medicationDispense.hasType() ? medicationDispense.getType().getText()
                                                        : null);
                                        if (medicationDispense.hasDosageInstruction()) {
                                            Dosage dosage = Iterables
                                                    .getLast(medicationDispense.getDosageInstruction());
                                            Map<String, Object> dosagePayload = new HashMap<>();
                                            ArrayList<BigDecimal> daysList = new ArrayList<>();
                                            ArrayList<String> schedules = new ArrayList<>();
                                            schedules.add(medicationDispense.hasWhenPrepared()
                                                    ? medicationDispense.getWhenPrepared().toString()
                                                    : null);
                                            schedules.add(medicationDispense.hasWhenHandedOver()
                                                    ? medicationDispense.getWhenHandedOver().toString()
                                                    : null);
                                            String quantity = "";
                                            if (medicationDispense.hasQuantity()) {
                                                String unit = medicationDispense.getQuantity().getUnit() == null ? ""
                                                        : medicationDispense.getQuantity().getUnit();
                                                quantity = medicationDispense.getQuantity().getValue()
                                                        + ("null".equals(unit) ? "" : (" " + unit));
                                            }
                                            dosagePayload.put("dose", quantity);
                                            dosagePayload.put("frequency",
                                                    dosage.hasTiming() && dosage.getTiming().hasRepeat()
                                                            ? dosage.getTiming().getRepeat().getFrequency()
                                                            : null);
                                            dosagePayload.put("route",
                                                    dosage.hasRoute() && dosage.getRoute().hasCoding()
                                                            ? dosage.getRoute().getCoding().get(0).getDisplay()
                                                            : null);
                                            dosagePayload.put("instructions",
                                                    dosage.hasText() ? dosage.getText() : null);
                                            dosagePayload.put("quantity",
                                                    medicationDispense.hasQuantity()
                                                            ? medicationDispense.getQuantity().getValue()
                                                            : null);
                                            dosagePayload.put("duration", duration);
                                            daysList.add(medicationDispense.hasDaysSupply()
                                                    ? medicationDispense.getDaysSupply().getValue()
                                                    : null);
                                ...
                                List<DiagnosticReport> diagnosticReportsList = getDiagnosticReportsByCategory(
                                        encounter.getIdElement().getIdPart(), "radiology-category");
                                if (!diagnosticReportsList.isEmpty()) {
                                    for (DiagnosticReport diagnosticReport : diagnosticReportsList) {
                                        ...
                                        radiologyDetailsDTO.setTestDate(
                                                diagnosticReport.hasIssued() ? diagnosticReport.getIssued() : null);
                                        radiologyDetailsDTO.setTestTypeName(
                                                diagnosticReport.hasCode() && diagnosticReport.getCode().hasCoding()
                                                        ? diagnosticReport.getCode().getCoding().get(0).getDisplay()
                                                        : null);
                                        ...
                                        radiologyDetailsDTO.setTestReport(
                                                diagnosticReport.hasConclusion() ? diagnosticReport.getConclusion()
                                                        : null);
                                        // TODO: Add testReport and bodySite
                                        String mediaReferenceId = diagnosticReport.hasMedia()
                                                ? diagnosticReport.getMedia().get(0).getLink().getReference()
                                                : null;
                                        if (mediaReferenceId != null) {
                                            Media media = fhirClient.read().resource(Media.class)
                                                    .withId(mediaReferenceId).execute();
                                            radiologyDetailsDTO
                                                    .setUrl(media.hasContent() ? media.getContent().getUrl() : null);
                                        }
                                ...
                                // Outcome details
                                List<Observation> outcomeObservations = getObservationsByCategory("outcome-details",
                                ...
                                if (!outcomeObservations.isEmpty()) {
                                    ...
                                    outcomeDetailsDTO
                                            .setInvestigationConducted(getComponentValueBoolean(observation, 4));
                                ...
                                List<Observation> causeOfDeathObservations = getObservationsByCategory("cause-of-death",
                                        encounter, false, true);
                                if (!causeOfDeathObservations.isEmpty()) {
                                    ...
                                    causesOfDeathDetailsDTO.setDateOfDeath(observation.hasEffectiveDateTimeType()
                                            ? observation.getEffectiveDateTimeType().getValue()
                                            : null);
                                    causesOfDeathDetailsDTO.setLineA(
                                            observation.hasComponent() && !observation.getComponent().isEmpty()
                                                    && observation.getComponent().get(0).hasValueStringType()
                                                            ? observation.getComponent().get(0).getValueStringType()
                                                                    .toString()
                                                            : null);
                                    ...
                                    causesOfDeathDetailsDTO.setLineB(
                                            observation.hasComponent() && observation.getComponent().size() > 1
                                                    && observation.getComponent().get(1).hasValueStringType()
                                                            ? observation.getComponent().get(1).getValueStringType()
                                                                    .toString()
                                                            : null);
                                    ...
                                    causesOfDeathDetailsDTO.setLineC(
                                            observation.hasComponent() && observation.getComponent().size() > 2
                                                    && observation.getComponent().get(2).hasValueStringType()
                                                            ? observation.getComponent().get(2).getValueStringType()
                                                                    .toString()
                                                            : null);
                                    ...
                                    causesOfDeathDetailsDTO.setLineD(
                                            observation.hasComponent() && observation.getComponent().size() > 3
                                                    && observation.getComponent().get(3).hasValueStringType()
                                                            ? observation.getComponent().get(3).getValueStringType()
                                                                    .toString()
                                                            : null);
                                    ...
                                    causesOfDeathDetailsDTO.setCauseOfDeathOther(
                                            observation.hasComponent() && observation.getComponent().size() > 4
                                                    && observation.getComponent().get(4).hasValueStringType()
                                                            ? observation.getComponent().get(4).getValueStringType()
                                                                    .toString()
                                                            : null);
                                    ...
                                    causesOfDeathDetailsDTO.setPlaceOfDeath(
                                            observation.hasComponent() && observation.getComponent().size() > 5
                                                    && observation.getComponent().get(5).hasValueStringType()
                                                            ? observation.getComponent().get(5).getValueStringType()
                                                                    .toString()
                                                            : null);
                                    ...
                                    causesOfDeathDetailsDTO.setMannerOfDeath(
                                            observation.hasComponent() && observation.getComponent().size() > 6
                                                    && observation.getComponent().get(6).hasValueStringType()
                                                            ? observation.getComponent().get(6).getValueStringType()
                                                                    .toString()
                                                            : null);
                                    ...
                                    otherDeathDetailsDTO.setPostmortemDetails(
                                            observation.hasComponent() && observation.getComponent().size() > 7
                                                    && observation.getComponent().get(7).hasValueStringType()
                                                            ? observation.getComponent().get(7).getValueStringType()
                                                                    .toString()
                                                            : null);
                                    ...
                                    otherDeathDetailsDTO.setMarcerated(
                                            observation.hasComponent() && observation.getComponent().size() > 8
                                                    ...
                                                    && observation.getComponent().get(8).getValueBooleanType()
                                                            .getValue() != null
                                                                    ? observation.getComponent().get(8)
                                                                            .getValueBooleanType().getValue()
                                                                    : null);
                                    ...
                                    otherDeathDetailsDTO.setFresh(
                                            observation.hasComponent() && observation.getComponent().size() > 9
                                                    ...
                                                    && observation.getComponent().get(9).getValueBooleanType()
                                                            .getValue() != null
                                                                    ? observation.getComponent().get(9)
                                                                            .getValueBooleanType().getValue()
                                                                    : null);
                                    ...
                                    otherDeathDetailsDTO.setMotherCondition(
                                            observation.hasComponent() && observation.getComponent().size() > 10
                                                    && observation.getComponent().get(10).hasValueStringType()
                                                            ? observation.getComponent().get(10).getValueStringType()
                                                                    .toString()
                                                            : null);
                                ...
                                List<Observation> antenatalCareObservations = getObservationsByCategory("anc-details",
                                        encounter, false, true);
                                if (!antenatalCareObservations.isEmpty()) {
                                    ...
                                    antenatalCareDetailsDTO.setDate(observation.hasEffectiveDateTimeType()
                                            ? observation.getEffectiveDateTimeType().getValue()
                                            : null);
                                    antenatalCareDetailsDTO
                                            .setPregnancyAgeInWeeks(getComponentValueQuantityInt(observation, 0) != null
                                                    ? getComponentValueQuantityInt(observation, 0).intValue()
                                                    : null);
                                    antenatalCareDetailsDTO.setPositiveHivStatusBeforeService(
                                            getComponentValueBoolean(observation, 1));
                                    antenatalCareDetailsDTO.setProvidedWithFamilyPlanningCounseling(
                                            getComponentValueBoolean(observation, 2));
                                    antenatalCareDetailsDTO.setProvidedWithInfantFeedingCounseling(
                                            getComponentValueBoolean(observation, 3));
                                    ...
                                    syphilisDetails.put("status",
                                            getComponentValueCodeableConceptDisplay(observation, 8));
                                    ...
                                    spouseSyphilisDetails
                                            .setStatus(getComponentValueCodeableConceptDisplay(observation, 7));
                                ...
                                List<Procedure> prophylAxisProcedures = getProceduresByCategoryAndObservationReference(
                                        encounter.getIdElement().getIdPart(), "prophyl-axis-details", null);
                                if (!prophylAxisProcedures.isEmpty()) {
                                    for (Procedure procedure : prophylAxisProcedures) {
                                        ...
                                        prophylAxisDetailsDTO.setDate(procedure.hasPerformedDateTimeType()
                                                ? procedure.getPerformedDateTimeType().getValue()
                                                : null);
                                        if (procedure.hasCode() && !procedure.getCode().getCoding().isEmpty()) {
                                            prophylAxisDetailsDTO
                                                    .setCode(procedure.getCode().getCoding().get(0).getCode());
                                        }
                                        prophylAxisDetailsDTO
                                                .setCode(procedure.hasCode() && procedure.getCode().hasText()
                                                        ? procedure.getCode().getText()
                                                        : null);
                                        prophylAxisDetailsDTO
                                                .setName(procedure.hasCode() && procedure.getCode().hasCoding()
                                                        && !procedure.getCode().getCoding().isEmpty()
                                                        && procedure.getCode().getCoding().get(0).hasDisplay()
                                                                ? procedure.getCode().getCoding().get(0).getDisplay()
                                                                : null);
                                        prophylAxisDetailsDTO.setStatus(
                                                procedure.hasStatus() ? procedure.getStatus().getDisplay() : null);
                                        prophylAxisDetailsDTO
                                                .setNotes(procedure.hasNote() && !procedure.getNote().isEmpty()
                                                        ? procedure.getNote().get(0).getText()
                                                        : null);
                                        ReactionDTO prophylAxisReaction = new ReactionDTO();
                                        if (procedure.hasExtension() && procedure.getExtension().isEmpty()) {
                                            prophylAxisReaction
                                                    .setReactionDate(getNestedExtensionValueDateTime(procedure,
                                                            "http://fhir.moh.go.tz/fhir/StructureDefinition/event-date",
                                                            "reactionDate"));
                                            prophylAxisReaction.setReported(getNestedExtensionValueBoolean(procedure,
                                                    "http://fhir.moh.go.tz/fhir/StructureDefinition/event-date",
                                                    "reported"));
                                            prophylAxisReaction.setNotes(getNestedExtensionValueString(procedure,
                                                    "http://fhir.moh.go.tz/fhir/StructureDefinition/event-date",
                                                    "reactionNotes"));
                                        }
                                ...
                                List<Procedure> chemotherapyProcedures = getProceduresByCategoryAndObservationReference(
                                        encounter.getIdElement().getIdPart(), "chemotherapy-treatment", null);
                                if (!chemotherapyProcedures.isEmpty()) {
                                    ...
                                    for (Procedure procedure : chemotherapyProcedures) {
                                        ...
                                        chemoTherapy.put("diagnosis",
                                                procedure.hasReasonCode() && !procedure.getReasonCode().isEmpty()
                                                        && procedure.getReasonCode().get(0).hasCoding()
                                                                ? procedure.getReasonCode().get(0).getCoding().get(0)
                                                                        .getDisplay()
                                                                : null);
                                        chemoTherapy.put("regiment", getNestedExtensionValueString(procedure,
                                                "http://fhir.moh.go.tz/fhir/StructureDefinition/chemotherapy-details",
                                                "regiment"));
                                        chemoTherapy.put("stage", getNestedExtensionValueInteger(procedure,
                                                "http://fhir.moh.go.tz/fhir/StructureDefinition/chemotherapy-details",
                                                "stage"));
                                        chemoTherapy.put("totalNumberOfExpectedCycles", getNestedExtensionValueInteger(
                                                procedure,
                                                "http://fhir.moh.go.tz/fhir/StructureDefinition/chemotherapy-details",
                                                "totalExpectedCycles"));
                                        chemoTherapy.put("currentChemotherapeuticCycles",
                                                getNestedExtensionValueInteger(procedure,
                                                        "http://fhir.moh.go.tz/fhir/StructureDefinition/chemotherapy-details",
                                                        "currentCycles"));
                                ...
                                List<Procedure> radiotherapyProcedures = getProceduresByCategoryAndObservationReference(
                                        encounter.getIdElement().getIdPart(), "radiotherapy-treatment", null);
                                if (!radiotherapyProcedures.isEmpty()) {
                                    ...
                                    for (Procedure procedure : radiotherapyProcedures) {
                                        ...
                                        prescription.put("type", getNestedExtensionValueString(procedure,
                                                "http://fhir.moh.go.tz/fhir/StructureDefinition/radiotherapy-details",
                                                "prescriptionType"));
                                        prescription.put("intention", getNestedExtensionValueString(procedure,
                                                "http://fhir.moh.go.tz/fhir/StructureDefinition/radiotherapy-details",
                                                "intention"));
                                        prescription.put("technique", getNestedExtensionValueString(procedure,
                                                "http://fhir.moh.go.tz/fhir/StructureDefinition/radiotherapy-details",
                                                "technique"));
                                        prescription.put("site", getNestedExtensionValueString(procedure,
                                                "http://fhir.moh.go.tz/fhir/StructureDefinition/radiotherapy-details",
                                                "site"));
                                        prescription.put("dailyDose", getNestedExtensionValueQuantityValue(procedure,
                                                "http://fhir.moh.go.tz/fhir/StructureDefinition/radiotherapy-details",
                                                "dailyDose"));
                                        prescription.put("totalDose", getNestedExtensionValueQuantityValue(procedure,
                                                "http://fhir.moh.go.tz/fhir/StructureDefinition/radiotherapy-details",
                                                "totalDose"));
                                        // TODO: Add the following values startDate, dosageDates, administrationDates
                                        // and remarks
                                        // Dosage dates
                                        ...
                                        if (procedure.hasReport()) {
                                            ...
                                            for (Reference reference : payLoadReports) {
                                                ...
                                                if (documentReference != null) {
                                                    report.put("date",
                                                            documentReference.hasDate() ? documentReference.getDate()
                                                                    : null);
                                                    // TODO: attachments is a string but has been saved as a list in the
                                                    // radiologytherapy treatment
                                                    // report.put("attachments", documentReference.hasAttachment() ?
                                                    // documentReference.getAttachment() : null);
                                                    report.put("MU", getNestedExtensionValueInteger(documentReference,
                                ...
                                List<Procedure> surgeryProcedures = getProceduresByCategoryAndObservationReference(
                                        encounter.getIdElement().getIdPart(), "surgery-treatment", null);
                                if (!surgeryProcedures.isEmpty()) {
                                    ...
                                    for (Procedure procedure : surgeryProcedures) {
                                        ...
                                        surgery.put("diagnosis",
                                                procedure.hasCode() && procedure.getCode().hasCoding()
                                                        ? procedure.getCode().getCoding().get(0).getDisplay()
                                                        : null);
                                        if (procedure.hasReasonCode() && !procedure.getReasonCode().isEmpty()) {
                                            surgery.put("reason", procedure.getReasonCode().get(0).getText());
                                        }
                                        surgeryReport.put("indication", getNestedExtensionValueString(procedure,
                                                "http://fhir.moh.go.tz/fhir/StructureDefinition/extension-procedure-report",
                                                "indication"));
                                        surgeryReport.put("steps", getNestedExtensionValueString(procedure,
                                                "http://fhir.moh.go.tz/fhir/StructureDefinition/extension-procedure-report",
                                                "steps"));
                                        surgeryReport.put("remarks", getNestedExtensionValueString(procedure,
                                                "http://fhir.moh.go.tz/fhir/StructureDefinition/extension-procedure-report",
                                                "remarks"));
                                        ...
                                        // System.out.println("*2*Reference object is missing ID and Reference: " +
                                        // reportPayload);
                                        // continue; // Skip this iteration
                                        // }
                                ...
                                List<Procedure> hormoneTherapyTreatments = getProceduresByCategoryAndObservationReference(
                                        encounter.getIdElement().getIdPart(), "hormonetherapy-treatment", null);
                                        ...
                                        treatment.put("diagnosis",
                                                procedure.hasReasonCode() && !procedure.getReasonCode().isEmpty()
                                                        ? procedure.getReasonCode().get(0).getCoding().get(0)
                                                                .getDisplay()
                                                        : null);
                                        treatment.put("regiment", getNestedExtensionValueString(procedure,
                                                "http://fhir.moh.go.tz/fhir/StructureDefinition/hormone-therapy-details",
                                                "regiment"));
                                        treatment.put("stage", getNestedExtensionValueInteger(procedure,
                                                "http://fhir.moh.go.tz/fhir/StructureDefinition/hormone-therapy-details",
                                                "stage"));
                                        treatment.put("totalNumberOfExpectedCycles", getNestedExtensionValueInteger(
                                                procedure,
                                                "http://fhir.moh.go.tz/fhir/StructureDefinition/hormone-therapy-details",
                                                "totalExpectedCycles"));
                                        treatment.put("currentCycles", getNestedExtensionValueInteger(procedure,
                                                "http://fhir.moh.go.tz/fhir/StructureDefinition/hormone-therapy-details",
                                                "currentCycles"));
                                ...
                                List<Procedure> medicalProcedures = getProceduresByCategoryAndObservationReference(
                                        encounter.getIdElement().getIdPart(), "medical-procedure-details", null);
                                if (!medicalProcedures.isEmpty()) {
                                    ...
                                    for (Procedure procedure : medicalProcedures) {
                                        ...
                                        treatment.setProcedureDate(procedure.hasPerformedDateTimeType()
                                                ? procedure.getPerformedDateTimeType().getValue()
                                                : null);
                                        treatment.setProcedureType(
                                                procedure.hasCode() ? procedure.getCode().getText() : null);
                                        treatment.setDiagnosis(
                                                procedure.hasReasonCode() && !procedure.getReasonCode().isEmpty()
                                                        ? procedure.getReasonCode().get(0).getText()
                                                        : null);
                                        treatment.setFindings(getNestedExtensionValueString(procedure,
                                ...
                                List<VaccinationDetailsDTO> vaccinationDetailsDTOS = getVaccinationDetails(
                                        encounter.getIdElement().getIdPart(), patient, null);
                                ...
                                if (!chargedItems.isEmpty()) {
                                    for (ChargeItem chargeItem : chargedItems) {
                                        ...
                                        billingsDetailsDTO
                                                .setBillType(chargeItem.hasReason() && !chargeItem.getReason().isEmpty()
                                                        ? chargeItem.getReason().get(0).getText()
                                                        : null);
                                        billingsDetailsDTO
                                                .setBillingCode(chargeItem.hasCode() && chargeItem.getCode().hasCoding()
                                                        && !chargeItem.getCode().getCoding().isEmpty()
                                                                ? chargeItem.getCode().getCoding().get(0).getCode()
                                                                : null);
                                        billingsDetailsDTO.setAmountBilled(
                                                chargeItem.hasPriceOverride() ? chargeItem.getPriceOverride().getValue()
                                                        : null);
                                        billingsDetailsDTO.setBillDate(chargeItem.hasOccurrenceDateTimeType()
                                                ? chargeItem.getOccurrenceDateTimeType().getValue()
                                                : null);
                                        billingsDetailsDTO.setExemptionType(getNestedExtensionValueString(chargeItem,
                                                "http://fhir.moh.go.tz/fhir/StructureDefinition/billing-details",
                                                "exemptionType"));
                                        billingsDetailsDTO.setStandardCode(getNestedExtensionValueString(chargeItem,
                                                "http://fhir.moh.go.tz/fhir/StructureDefinition/billing-details",
                                                "standardCode"));
                                ...
                                List<CarePlan> carePlans = getCarePlansByCategory(encounter.getIdElement().getIdPart(),
                                        "family-planning");
                                if (!carePlans.isEmpty()) {
                                    // TODO: Decide on what item should be used last
                                    CarePlan carePlan = Iterables.getLast(carePlans);
                                    familyPlanningDetailsDTO
                                            .setDate(carePlan.hasPeriod() ? carePlan.getPeriod().getStart() : null);
                                    ...
                                    if (carePlan.hasActivity() && !carePlan.getActivity().isEmpty()) {
                                        for (CarePlan.CarePlanActivityComponent activity : carePlan.getActivity()) {
                                            if (activity.hasDetail() && activity.getDetail().hasCode()
                                                    && activity.getDetail().getCode().hasCoding()
                                                    && activity.getDetail().getCode().getCoding().size() > 1) {
                                                if (activity.getDetail().getCode().getCoding().get(1).getCode()
                                                        .equals("long-term-method")) {
                                                    LongTermMethodDTO longTermMethodDTO = new LongTermMethodDTO();
                                                    longTermMethodDTO.setProvided(activity.getDetail().getCode()
                                                            .getCoding().get(0).hasCode());
                                                    longTermMethodDTO.setCode(activity.getDetail().getCode().getCoding()
                                                            .get(0).getCode());
                                                    longTermMethodDTO.setType(activity.getDetail().getCode().getCoding()
                                                            .get(0).getDisplay());
                                                ...
                                                if (activity.getDetail().getCode().getCoding().get(1).getCode()
                                                    ...
                                                    shortTermMethodDTO.setProvided(activity.getDetail().getCode()
                                                            .getCoding().get(0).hasCode());
                                                    shortTermMethodDTO.setCode(activity.getDetail().getCode()
                                                            .getCoding().get(0).getCode());
                                                    shortTermMethodDTO.setType(activity.getDetail().getCode()
                                ...
                                List<Procedure> deliveryProcedures = getProceduresByCategoryAndObservationReference(
                                        encounter.getIdElement().getIdPart(), "obstetrics", null);
                                if (!deliveryProcedures.isEmpty()) {
                                    ...
                                    laborAndDeliveryDetailsDTO.setDate(deliveryProcedure.hasPerformedDateTimeType()
                                            ? deliveryProcedure.getPerformedDateTimeType().getValue()
                                            : null);
                                    Map<String, Object> deliveryMethod = new HashMap<>();
                                    if (deliveryProcedure.hasCode() && deliveryProcedure.getCode().hasCoding()
                                            && !deliveryProcedure.getCode().getCoding().isEmpty()) {
                                        deliveryMethod.put("code",
                                                deliveryProcedure.getCode().getCoding().get(0).getCode());
                                        deliveryMethod.put("name",
                                                deliveryProcedure.getCode().getCoding().get(0).getDisplay());
                                        laborAndDeliveryDetailsDTO.setDeliveryMethod(deliveryMethod);
                                    }
                                    if (deliveryProcedure.hasExtension()
                                            && !deliveryProcedure.getExtension().isEmpty()) {
                                        for (Extension extension : deliveryProcedure.getExtension()) {
                                            if (extension.hasUrl() && extension.getUrl().equals(
                                                    "http://fhir.moh.go.tz/fhir/StructureDefinition/placeOfBirth")) {
                                                laborAndDeliveryDetailsDTO.setPlaceOfBirth(extension.hasValue()
                                            ...
                                            if (extension.hasUrl() && extension.getUrl().equals(
                                                ...
                                                laborAndDeliveryDetailsDTO
                                                        .setTimeBetweenLaborPainAndDeliveryInHrs(extension.hasValue()
                                                                && extension.getValue() instanceof DecimalType
                                                                        ? ((DecimalType) extension.getValue())
                                                                                .getValue().intValue()
                                                                        : null);
                                            }
                                            if (extension.hasUrl() && extension.getUrl().equals(
                                                    "http://fhir.moh.go.tz/fhir/StructureDefinition/isAttendantSkilled")) {
                                                laborAndDeliveryDetailsDTO.setIsAttendantSkilled(extension.hasValue()
                                ...
                                List<Observation> infantFeedingCounselings = getObservationsByCategory(
                                        "infant-feeding-counseling", encounter, false, true);
                                List<Observation> familyPlanningCounselings = getObservationsByCategory(
                                        "family-planning-counseling", encounter, false, true);
                                if (!infantFeedingCounselings.isEmpty()) {
                                    Observation infantFeedingCounseling = infantFeedingCounselings.get(0);
                                    if (infantFeedingCounseling != null && infantFeedingCounseling.hasValueBooleanType()
                                            && infantFeedingCounseling.getValueBooleanType().hasValue()) {
                                        laborAndDeliveryDetailsDTO.setProvidedWithInfantFeedingCounseling(
                                                infantFeedingCounseling.getValueBooleanType().getValue());
                                    }
                                }
                                if (!familyPlanningCounselings.isEmpty()) {
                                    ...
                                    if (familyPlanningCounseling != null
                                            && familyPlanningCounseling.hasValueBooleanType()
                                            && familyPlanningCounseling.getValueBooleanType().hasValue()) {
                                        laborAndDeliveryDetailsDTO.setProvidedWithFamilyPlanningCounseling(
                                                familyPlanningCounseling.getValueBooleanType().getValue());
                                    }
                                }
                                ...
                                List<Condition> beforeBirthComplicationConditions = getConditionsByCategory(
                                        encounter.getIdElement().getIdPart(), "before-birth-complication");
                                ...
                                List<Condition> birthComplicationConditions = getConditionsByCategory(
                                        encounter.getIdElement().getIdPart(), "birth-complication");
                                if (!birthComplicationConditions.isEmpty()) {
                                    for (Condition condition : beforeBirthComplicationConditions) {
                                        CodeAndNameDTO birthComplication = new CodeAndNameDTO();
                                        birthComplication.setCode(condition.hasCode() && condition.getCode().hasCoding()
                                                && !condition.getCode().getCoding().isEmpty()
                                                        ? condition.getCode().getCoding().get(0).getCode()
                                                        : null);
                                        birthComplication.setName(condition.hasCode() && condition.getCode().hasCoding()
                                                && !condition.getCode().getCoding().isEmpty()
                                                        ? condition.getCode().getCoding().get(0).getDisplay()
                                                        : null);
                                ...
                                List<Observation> birthDetailsObservations = getObservationsByCategory(
                                        "labor-delivery-birth-details", encounter, false, false);
                                ...
                                if (!birthDetailsObservations.isEmpty()) {
                                    for (Observation observation : birthDetailsObservations) {
                                        ...
                                        birthDetailsDTO.setDateOfBirth(getNestedExtensionValueDateTime(observation,
                                                "http://fhir.moh.go.tz/fhir/StructureDefinition/newborn-birth-details",
                                                "dateOfBirth"));
                                        birthDetailsDTO.setExclusiveBreastFed(getNestedExtensionValueBoolean(
                                                observation,
                                                "http://fhir.moh.go.tz/fhir/StructureDefinition/newborn-birth-details",
                                                "exclusiveBreastFed"));
                                        birthDetailsDTO.setMotherAgeInYears(getNestedExtensionValueInteger(observation,
                                        ...
                                        birthDetailsDTO.setProvidedWithARV(getNestedExtensionValueBoolean(observation,
                                                "http://fhir.moh.go.tz/fhir/StructureDefinition/maternal-details",
                                                "providedWithARV"));
                                        birthDetailsDTO
                                                .setWeightInKgs(getComponentValueQuantityInt(observation, 0) != null
                                                        ? getComponentValueQuantityInt(observation, 0).floatValue()
                                                        : null);
                                        ...
                                        List<VaccinationDetailsDTO> vaccinationDetails = getVaccinationDetails(
                                                encounter.getIdElement().getIdPart(), patient,
                                        ...
                                        List<Procedure> procedures = getProceduresByCategoryAndObservationReference(
                                                encounter.getIdElement().getIdPart(), "breathe-assistance",
                                                observation.getIdElement().getIdPart());
                                        if (!procedures.isEmpty()) {
                                            Procedure procedure = Iterables.getLast(procedures);
                                            breatheAssistanceDTO
                                                    .setCode(procedure.hasCode() && procedure.getCode().hasCoding()
                                                            && !procedure.getCode().getCoding().isEmpty()
                                                                    ? procedure.getCode().getCoding().get(0).getCode()
                                                                    : null);
                                            breatheAssistanceDTO
                                                    .setProvided(procedure.hasCode() && procedure.getCode().hasCoding()
                                                            && !procedure.getCode().getCoding().isEmpty());
                                ...
                                List<Observation> postnatalDetailsObservations = getObservationsByCategory(
                                        "postnatal-details", encounter, false, true);
                                if (!postnatalDetailsObservations.isEmpty()) {
                                    Observation postnatalDetailObservation = postnatalDetailsObservations.get(0);
                                    postnatalDetailsDTO.setDate(postnatalDetailObservation.hasEffectiveDateTimeType()
                                            ? postnatalDetailObservation.getEffectiveDateTimeType().getValue()
                                            : null);
                                    postnatalDetailsDTO.setPositiveHivStatusBeforeService(
                                            getComponentValueBoolean(postnatalDetailObservation, 0));
                                    postnatalDetailsDTO
                                            .setReferredToCTC(getComponentValueBoolean(postnatalDetailObservation, 1));
                                    postnatalDetailsDTO.setReferredToClinicForFurtherServices(
                                            getComponentValueBoolean(postnatalDetailObservation, 2));
                                    postnatalDetailsDTO
                                            .setOutCome(getComponentValueString(postnatalDetailObservation, 3));
                                    postnatalDetailsDTO
                                            .setAPGARScore(getComponentIntValue(postnatalDetailObservation, 4));
                                    // TODO: Consider declaring these values first before using them in the
                                    // condition
                                    if (getComponentValueBoolean(postnatalDetailObservation, 5) != null) {
                                        ProvidedAndCodeDTO demagedNipples = new ProvidedAndCodeDTO();
                                        demagedNipples
                                                .setProvided(getComponentValueBoolean(postnatalDetailObservation, 5));
                                    ...
                                    if (getComponentValueBoolean(postnatalDetailObservation, 7) != null) {
                                        ...
                                        breastAbscess
                                                .setProvided(getComponentValueBoolean(postnatalDetailObservation, 7));
                                    ...
                                    if (getComponentValueBoolean(postnatalDetailObservation, 9) != null) {
                                        ...
                                        puerperalPsychosis
                                                .setProvided(getComponentValueBoolean(postnatalDetailObservation, 9));
                                    ...
                                    postnatalDetailsDTO.setHoursSinceDelivery(
                                            getComponentIntValue(postnatalDetailObservation, 10));
                                    ...
                                    List<Observation> birthDetailsPostnatalObservations = getObservationsByCategory(
                                            "postnatal-birth-details", encounter, false, false);
                                    ...
                                    if (!birthDetailsPostnatalObservations.isEmpty()) {
                                        for (Observation observation : birthDetailsPostnatalObservations) {
                                            ...
                                            birthDetailsDTO.setExclusiveBreastFed(getNestedExtensionValueBoolean(
                                                    ...
                                                    "http://fhir.moh.go.tz/fhir/StructureDefinition/newborn-birth-details",
                                                    "exclusiveBreastFed"));
                                            birthDetailsDTO.setMotherAgeInYears(getNestedExtensionValueInteger(
                                                    observation,
                                                    "http://fhir.moh.go.tz/fhir/StructureDefinition/maternal-details",
                                                    "motherAgeInYears"));
                                            // TODO: Add Mother HIV status
                                            ...
                                            // "http://fhir.moh.go.tz/fhir/StructureDefinition/maternal-details",
                                            // "motherHivStatus"));
                                            birthDetailsDTO.setProvidedWithARV(getNestedExtensionValueBoolean(
                                                    observation,
                                                    "http://fhir.moh.go.tz/fhir/StructureDefinition/maternal-details",
                                                    "providedWithARV"));
                                            birthDetailsDTO
                                            ...
                                            if (!procedures.isEmpty()) {
                                                Procedure procedure = Iterables.getLast(procedures);
                                                breatheAssistanceDTO.setCode(procedure.hasCode()
                                                        && procedure.getCode().hasCoding()
                                                        && !procedure.getCode().getCoding().isEmpty()
                                                                ? procedure.getCode().getCoding().get(0).getCode()
                                                                : null);
                                                breatheAssistanceDTO.setProvided(
                                                        procedure.hasCode() && procedure.getCode().hasCoding()
                                                                && !procedure.getCode().getCoding().isEmpty());
                                                birthDetailsDTO.setBreatheAssistance(breatheAssistanceDTO);
                                            }
                                ...
                                List<Observation> admissionDetailObservations = getObservationsByCategory(
                                        "admission-details", encounter, false, true);
                                if (!admissionDetailObservations.isEmpty()) {
                                    Observation admissionDetail = admissionDetailObservations.get(0);
                                    admissionDetailsDTO.setAdmissionDate(admissionDetail.hasEffectiveDateTimeType()
                                            && admissionDetail.getEffectiveDateTimeType().hasValue()
                                                    ? admissionDetail.getEffectiveDateTimeType().getValue()
                                                    : null);
                                    admissionDetailsDTO.setAdmissionDiagnosis(
                                            getComponentValueCodeableConceptCode(admissionDetail, 1));
                            ...
                            try {
                                ...
                                if (facilityConnectionDetails != null) {
                                    ...
                                    Map<String, Object> emrHealthRecords = mediatorsService.routeToMediator(
                                            facilityConnectionDetails,
                                            "emrHealthRecords?id=" + identifier + "&idType=" + identifierType, "GET",
                                            null);
    ...
    public List<Encounter> getLatestEncounterUsingPatientAndOrganisation(String id, Organization organization,
        ...
        results = encounterSearch.sort(new SortSpec("date").setOrder(SortOrderEnum.DESC)).returnBundle(Bundle.class)
                .execute();
    ...
    public List<Observation> getObservationsByCategory(String category, Encounter encounter, boolean forGroup,
        ...
        if (fetchLastUpdated) {
            ...
            observationBundle = observationSearch.sort().descending("_lastUpdated").returnBundle(Bundle.class)
                    .execute();
        }
    ...
    public List<Observation> getObservationsByObservationGroupId(String category, Encounter encounter, String id)
            throws Exception {
        List<Observation> observations = new ArrayList<>();
        var observationSearch = fhirClient.search().forResource(Observation.class)
                .where(Observation.ENCOUNTER.hasAnyOfIds(encounter.getIdElement().getIdPart()));
    ...
    public List<Condition> getConditionsByCategory(String encounterId, String category) throws Exception {
        ...
        var conditionSearch = fhirClient.search().forResource(Condition.class)
                .where(Condition.ENCOUNTER.hasAnyOfIds(encounterId)).where(Condition.CATEGORY.exactly().code(category));
    ...
    public List<Procedure> getProceduresByCategoryAndObservationReference(String encounterId, String category,
            String observationId) throws Exception {
        List<Procedure> procedures = new ArrayList<>();
        var procedureSearch = fhirClient.search().forResource(Procedure.class)
                .where(Procedure.ENCOUNTER.hasAnyOfIds(encounterId)).where(Procedure.CATEGORY.exactly().code(category));
        ...
        if (observationBundle.hasEntry()) {
            for (Bundle.BundleEntryComponent entryComponent : observationBundle.getEntry()) {
                ...
                if (observationId != null) {
                    if (procedure.hasReasonReference() && !procedure.getReasonReference().isEmpty() && procedure
                            .getReasonReference().get(0).getReference().equals("Observation/" + observationId)) {
                    ...
                    if (resource instanceof Immunization) {
                        ...
                        if (observationId != null) {
                            boolean matchesObservation = immunization.hasReasonReference()
                                    && !immunization.getReasonReference().isEmpty() && immunization.getReasonReference()
                                            .get(0).getReference().equals("Observation/" + observationId);
    ...
    public List<CarePlan> getCarePlansByCategory(String encounterId, String category) throws Exception {
        ...
        var carePlanSearch = fhirClient.search().forResource(CarePlan.class)
                .where(CarePlan.ENCOUNTER.hasAnyOfIds(encounterId)).where(CarePlan.CATEGORY.exactly().code(category));
    ...
    public List<VaccinationDetailsDTO> getVaccinationDetails(
        ...
        // reference
        List<Immunization> vaccinationDetails = getImmunizationsByEncounterIdAndObservationReference(encounterId,
            ...
            for (Extension parentExtension : resource.getExtension()) {
                if (parentExtension.getUrl().equals(parentUrl) && parentExtension.hasExtension()) {
                    for (Extension childExtension : parentExtension.getExtension()) {
                        if (childExtension.getUrl().equals(childUrl) && childExtension.hasValue()
