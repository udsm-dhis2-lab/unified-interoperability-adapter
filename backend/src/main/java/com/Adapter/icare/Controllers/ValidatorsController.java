...
public class ValidatorsController {
    ...
    public ResponseEntity<Map<String, Object>> addValidator(@Valid @RequestBody Map<String, Object> validatorDTO) {
        try {
            DynamicValidator dynamicValidator = new DynamicValidator().fromMap(validatorDTO);
            if (this.authentication != null && this.authenticatedUser != null) {
                dynamicValidator.setCreatedBy(this.authenticatedUser);
            }
            return ResponseEntity.ok(validatorService.addNewValidator(dynamicValidator).toMap());
    ...
    public ResponseEntity<Map<String, Object>> updateValidator(@PathVariable("uuid") String uuid,
        ...
        try {
            DynamicValidator dynamicValidator = new DynamicValidator().fromMap(validatorDTO);
            if (dynamicValidator.getUuid() == null) {
                dynamicValidator.setUuid(uuid);
                if (this.authentication != null && this.authenticatedUser != null) {
                    dynamicValidator.setLastUpdatedBy(this.authenticatedUser);
                }
            }
            return ResponseEntity.ok(validatorService.updateValidator(dynamicValidator).toMap());
        } catch (DataFormatException e){
