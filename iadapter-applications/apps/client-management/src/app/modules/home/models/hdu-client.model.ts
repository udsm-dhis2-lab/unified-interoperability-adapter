export class HduClient {
  ...
  generateRandomId(inputDate: string): string {
    ...
    const sequencePart = Math.floor(Math.random() * 10000000)
      .toString()
      .padStart(7, '0'); // 7 digits with leading zeros
  ...
  static fromJson(json: any): HduClient {
    ...
    hduClient.fullName = `${json['firstName'] || ''} ${
      json['middleName'] || ''
    } ${json['lastName'] || ''}`;
    ...
    hduClient.files = json['files'];
    hduClient.nida = json['identifiers']
      .filter((id: any) => id['type'] === 'NIDA')
      .map((id: any) => id['id']);
