...
import {
  Workflow,
  WorkflowFormCreate,
} from '../../../workflow/models/workflow.model';
...
import { getAddedScheduleStatus } from '../../state/schedule/schedule.selectors';
    ...
    if (this.workflowForm.valid) {
      this.scheduleState.dispatch(
        ScheduleActions.addSchedule({
          schedule: this.workflowForm.value as Schedule,
        })
      );

      this.scheduleState
        .pipe(select(getAddedScheduleStatus), skip(1), take(1))
